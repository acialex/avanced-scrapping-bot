1. Go to https://proxyland.io/ and register. There is a free trial for testing purposes.
2. After registering create a package and go to the File create_chrome_plugin.py and replace PROXY_USER and PROXY_PASS with your package user name/password
3. run python create_chrome_plugin.py to create the plugin chrome will use for ip rotation
4. Now you can run the crawler with "python pasion_com.py". The result will be written to pasion.csv

This would probably also be possible with https://botproxy.net

------------------------------------------------

NEWS ---

Passion.com is not longer available.  You are free to use and rewritte the script for your own scrapping pourposes. If you need help you are free to ask me, I m able to scrappe tons of data bse on demand. Tell me what you need and I ll give u a good price.

My services:

- Scrapping
- Cooding on demand
- Websitte and app development
- Pentesting and forensic services
- logo desing
- Online marketing
- Copywritting services
- Restore old pictures




aci.alex87@gmail.com


Best Regards.
